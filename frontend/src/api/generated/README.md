...
WP-04 does not add a Session/bootstrap client or any business request. The
existing framework-light `src/api/client.ts` remains the only transport helper,
and future endpoint modules must continue using same-origin credentials rather
than the `/api/v1` machine API key.
