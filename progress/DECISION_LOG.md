...
| D-039 | 2026-08-23 | Insert ADJ-001 before WP-05 to enforce responsibility/source-size governance and verify real `/ui/` initial-route gzip improvement | Superseded in gating effect by D-040 | Phase A was completed; runtime measurement remains pending |
| D-040 | 2026-08-25 | Accept and integrate ADJ-001 Phase A with high technical remediation confidence; defer Phase B npm/Vite measurement and authorize WP-05 to start | **Owner accepted with explicit verification debt** | Five routes are lazy, Home removes `EuiCodeBlock`/Prism, AppShell remains the synchronous boundary, and the analyzer prevents filename-only false passes. This is not a measured Pass: 510,780 bytes remains the verified baseline. Phase B must pass before G2 closure or React production release; a measured value `>=510780` reopens ADJ-001 as blocking. |
