...
1. foundation success envelope;
2. process-only health and explicit `not_checked` dependency state;
3. routing without `PATH_INFO`;
4. HEAD behavior;
5. OPTIONS and route-specific `Allow`;
6. unknown-route 404;
7. known-route 405;
8. unsupported methods such as TRACE;
9. malformed JSON;
10. top-level JSON arrays and scalars;
11. unsupported Content-Type;
12. `application/json; charset=utf-8`;
13. `application/*+json`;
14. optional empty command bodies;
15. request/correlation ID validation, propagation and fallback;
16. raw/encoded/double-encoded separator and dot-segment rejection;
17. malformed percent encoding;
18. typed route-parameter failure and coercion;
19. generic 500 responses without stack, path, exception message or secret leakage;
20. 401, 403, 409, 422 and 429/`Retry-After` semantics;
21. no CORS grant;
22. no executable Session, DB-config or machine-API authentication binding;
23. OpenAPI envelope/status schema presence.
