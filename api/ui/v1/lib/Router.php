...
final class UiApiRoute
{
    ...
    public function __construct(string $method, string $pattern, string $name, callable $handler, array $parameterTypes = []) { $this->method = strtoupper($method); $this->pattern = $pattern; $this->name = $name; $this->handler = $handler; $this->parameterTypes = $parameterTypes; }
...
final class UiApiResponseFactory
{
    ...
    private function encode(array $payload): string { try { return json_encode($payload, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR); } catch (JsonException $e) { error_log('UI API response encoding failure: ' . $e->getMessage()); return '{"error":{"code":"server_error","message":"An unexpected server error occurred."}}'; } }
}

final class UiApiRouter
{
    private array $routes;
    public function __construct(array $routes) { foreach ($routes as $route) if (!$route instanceof UiApiRoute) throw new InvalidArgumentException('Every UI API route must be a UiApiRoute.'); $this->routes = array_values($routes); }
    public function dispatch(UiApiRequest $request, UiApiRequestContext $context, UiApiResponseFactory $responses): UiApiHttpResponse {
        $matches = []; $allowed = [];
        foreach ($this->routes as $route) { $parameters = $route->match($request->path()); if ($parameters === null) continue; $matches[] = [$route, $parameters]; $allowed[] = $route->method(); if ($route->method() === 'GET') $allowed[] = 'HEAD'; }
        ...
        foreach ($matches as $match) { $route = $match[0]; if ($route->method() !== $effective) continue; $result = $route->invoke($request, $context, $match[1]); if ($result instanceof UiApiHttpResponse) $response = $result; elseif (is_array($result)) $response = $responses->success($result, $context); else throw new RuntimeException('Route handler returned an unsupported response type.'); return $request->method() === 'HEAD' ? $response->withoutBody() : $response; }
        throw UiApiException::methodNotAllowed($allowed);
    }
    private function sortMethods(array $methods): array { $order = ['GET', 'HEAD', 'POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS']; usort($methods, static function (string $left, string $right) use ($order): int { $a = array_search($left, $order, true); $b = array_search($right, $order, true); return ($a === false ? 999 : $a) <=> ($b === false ? 999 : $b); }); return $methods; }
}
...
final class UiApiKernel
{
    ...
    public function handle(UiApiRequest $request): UiApiHttpResponse { $context = UiApiRequestContext::fromRequest($request); try { return $this->router->dispatch($request, $context, $this->responses); } catch (UiApiException $e) { return $this->responses->error($e, $context); } catch (Throwable $e) { error_log(sprintf('UI API unexpected error request_id=%s correlation_id=%s method=%s path=%s type=%s', $context->requestId(), $context->correlationId(), $request->method(), $request->path(), get_class($e))); return $this->responses->serverError($context); } }
    public function handleServer(array $server, array $query = [], ?string $body = null): UiApiHttpResponse { try { return $this->handle(UiApiRequest::fromServer($server, $query, $body)); } catch (UiApiException $e) { return $this->responses->error($e, UiApiRequestContext::fromServer($server)); } catch (Throwable $e) { $context = UiApiRequestContext::fromServer($server); error_log(sprintf('UI API request-construction error request_id=%s correlation_id=%s type=%s', $context->requestId(), $context->correlationId(), get_class($e))); return $this->responses->serverError($context); } }
}
