...
final class UiApiRequestContext
{
    ...
    public function withSecurityState(array $state): self
    {
        ...
        $copy->availableTenants = isset($state['availableTenants']) && is_array($state['availableTenants']) ? array_values($state['availableTenants']) : [];
        $copy->capabilities = isset($state['capabilities']) && is_array($state['capabilities']) ? array_values($state['capabilities']) : [];
        $copy->modules = isset($state['modules']) && is_array($state['modules']) ? array_values($state['modules']) : [];
        $copy->accessibleTenantIds = isset($state['accessibleTenantIds']) && is_array($state['accessibleTenantIds']) ? array_values(array_map('intval', $state['accessibleTenantIds'])) : [];
    ...
    {
        if (!$this->authenticated) return ['actor' => 'anonymous', 'tenant' => 'anonymous', 'capabilities' => 'anonymous', 'locale' => 'unresolved', 'timezone' => 'unresolved'];
        return ['actor' => $this->actor === null ? 'unresolved' : 'resolved', 'tenant' => $this->tenant === null ? 'unresolved' : 'resolved', 'capabilities' => 'resolved', 'locale' => $this->locale === null ? 'unresolved' : 'resolved', 'timezone' => $this->timezone === null ? 'unresolved' : 'resolved'];
    }
    ...
    private static function validId(?string $value): bool
    {
        ...
        $value = trim($value);
        return $value !== '' && strlen($value) <= 128 && preg_match('/^[A-Za-z0-9][A-Za-z0-9._:-]*$/', $value) === 1;
    }

    private static function generateId(): string
    {
        try { $bytes = random_bytes(16); } catch (Throwable $error) { $bytes = hash('sha256', uniqid('', true) . mt_rand(), true); }
