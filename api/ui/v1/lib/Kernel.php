...
final class UiApiKernel
{
    ...
    public function router(): UiRouter { return $this->router; }
    ...
    public function handleGlobals(): UiHttpResponse
    {
        try {
            ...
            return $this->responses->fromException(new UiApiException(500, 'server_error', 'An unexpected server error occurred.'), $context);
        }
    }
    ...
    public function handle(UiRequest $request): UiHttpResponse
    {
        ...
        try {
            ...
            error_log('UI API v1 handler failure [' . $context->requestId . '] ' . $request->method() . ' ' . $request->path() . ': ' . $e->getMessage() . ' @ ' . $e->getFile() . ':' . $e->getLine());
            return $this->responses->fromException(new UiApiException(500, 'server_error', 'An unexpected server error occurred.'), $context);
        }
    }
}
