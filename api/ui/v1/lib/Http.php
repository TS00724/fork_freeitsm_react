...
final class UiApiException extends RuntimeException
{
    ...
    public static function methodNotAllowed(array $allowed): self {
        return new self(405, 'method_not_allowed', 'The HTTP method is not allowed for this endpoint.', null, ['Allow' => implode(', ', $allowed)]);
    }
    ...
    public static function rateLimited(?int $retryAfter = null): self {
        return new self(429, 'rate_limited', 'Too many requests.', null, $retryAfter === null ? [] : ['Retry-After' => (string)$retryAfter]);
    }
}
...
final class UiApiRequest
{
    ...
    private function __construct(string $method, string $path, array $headers, array $query, string $body) {
        $this->method = $method; $this->path = $path; $this->headers = $headers; $this->query = $query; $this->rawBody = $body;
    }
    ...
    private static function containsControl(string $value): bool {
        for ($i = 0, $length = strlen($value); $i < $length; $i++) { $ord = ord($value[$i]); if ($ord <= 31 || $ord === 127) return true; }
        return false;
    }
}
...
final class UiApiRequestContext
{
    ...
    public function __construct(string $requestId, string $correlationId, ?string $timestamp = null, $actor = null, $tenant = null, $capabilities = null, ?string $locale = null, ?string $timezone = null) {
        $this->requestId = $requestId; $this->correlationId = $correlationId; $this->timestamp = $timestamp ?? gmdate('Y-m-d\TH:i:s\Z');
        $this->actor = $actor; $this->tenant = $tenant; $this->capabilities = $capabilities; $this->locale = $locale; $this->timezone = $timezone;
    }
    ...
    private static function fromIds(?string $request, ?string $correlation): self {
        ...
        return new self($requestId, self::validId($correlation) ? trim((string)$correlation) : $requestId);
    }
    ...
    private static function generateId(): string {
        try { $bytes = random_bytes(16); } catch (Throwable $e) { $bytes = hash('sha256', uniqid('', true) . mt_rand(), true); }
        $bytes[6] = chr((ord($bytes[6]) & 0x0f) | 0x40); $bytes[8] = chr((ord($bytes[8]) & 0x3f) | 0x80); $hex = bin2hex(substr($bytes, 0, 16));
