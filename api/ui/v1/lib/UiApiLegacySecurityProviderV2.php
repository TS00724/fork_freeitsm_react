...
final class UiApiLegacySecurityProviderV2 implements UiApiSecurityProvider
{
    private UiApiLegacyDatabase $database;
    private UiApiLegacyActor $actor;
    ...
    public function __construct(?UiApiLegacyDatabase $database = null)
    {
        ...
        $this->actor = new UiApiLegacyActor($this->database);
    }

    public function resolve(int $analystId, UiApiSessionStore $session): ?array
    {
        $actorRow = $this->actor->resolve($analystId);
    ...
    {
        if ($this->authorization === null) $this->authorization = new UiApiLegacyAuthorization($this->database->connection());
        return $this->authorization;
    }

    private function tenancy(): UiApiLegacyTenancy
    {
        if ($this->tenancy === null) $this->tenancy = new UiApiLegacyTenancy($this->database->connection());
        return $this->tenancy;
    }
