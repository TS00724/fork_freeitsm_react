...
final class UiResponseFactory
{
    ...
    public function fromRouteResult(UiRouteResult $result, UiRequestContext $context): UiHttpResponse
    {
        ...
        return $this->json($result->status, ['data' => $result->data, 'meta' => $meta], $context, $result->headers);
    }
    ...
    public function fromException(UiApiException $exception, UiRequestContext $context): UiHttpResponse
    {
        ...
        return $this->json($status, ['error' => $error, 'meta' => $this->standardMeta($context)], $context, $exception->responseHeaders());
    }
    ...
    private function json(int $status, array $payload, UiRequestContext $context, array $extraHeaders): UiHttpResponse
    {
        try {
            ...
            $fallback = [
                ...
                'meta' => $this->standardMeta($context),
            ];
            $encoded = json_encode($fallback, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
            $body = is_string($encoded) ? $encoded : '{"error":{"code":"server_error","message":"An unexpected server error occurred."}}';
        }
    ...
    private function headers(UiRequestContext $context, array $extra, bool $json): array
    {
        ...
        foreach ($extra as $name => $value) {
            if (!preg_match('/^[A-Za-z0-9-]+$/', $name) || preg_match('/[\x00-\x1F\x7F]/', $value)) {
                throw new \RuntimeException('Unsafe response header was rejected.');
            }
            $headers[$name] = $value;
        }
        return $headers;
