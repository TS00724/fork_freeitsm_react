        ...
        foreach ($this->templateSegments as $index => $templateSegment) {
            ...
            if ($type === 'int') {
                if (!preg_match('/^[1-9][0-9]*$/', $actual) || !self::fitsPositiveInteger($actual)) {
                    throw new UiApiException(400, 'invalid_route_parameter', "Route parameter '{$name}' must be a positive integer.", ['parameter' => $name]);
                }
                $params[$name] = (int)$actual;
                continue;
            }
            if ($type === 'slug' && !preg_match('/^[A-Za-z0-9][A-Za-z0-9_-]{0,127}$/', $actual)) {
                throw new UiApiException(400, 'invalid_route_parameter', "Route parameter '{$name}' is invalid.", ['parameter' => $name]);
            }
