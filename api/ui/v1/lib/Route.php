    ...
    public function match(string $path): ?array
    {
        ...
        foreach ($this->parameterTypes as $name => $type) {
            if (!array_key_exists($name, $parameters)) {
                throw UiApiException::badRequest('invalid_route_parameter', 'A required route parameter is missing.', ['parameter' => $name]);
            }
    ...
    private function coerceParameter(string $name, string $value, string $type)
    {
        if ($type === 'positive_int') {
            if ($value === '' || !ctype_digit($value) || (int)$value <= 0) {
                throw UiApiException::badRequest('invalid_route_parameter', 'A route parameter is invalid.', [
                    'parameter' => $name,
