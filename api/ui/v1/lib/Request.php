...
final class UiApiRequest
{
    ...
    public function jsonObject(): array
    {
        ...
        if (!$jsonType) throw UiApiException::unsupportedMediaType();
            ...
            if (is_string($requestPath) && $requestPath !== '') {
                if ($scriptName !== '' && strpos($requestPath, $scriptName) === 0) {
                    ...
                    if ($scriptDirectory !== '' && strpos($requestPath, $scriptDirectory . '/') === 0) {
                        $path = substr($requestPath, strlen($scriptDirectory));
                    }
                }
            }
        }
    ...
    private static function extractHeaders(array $server): array
    {
        ...
        if (isset($server['CONTENT_LENGTH'])) $headers['content-length'] = trim((string)$server['CONTENT_LENGTH']);
        return $headers;
    }
