...
wp04Test('OpenAPI declares required semantics', static function (): void {
    $contract=json_decode((string)file_get_contents(dirname(__DIR__).'/openapi-v1.json'),true);
    wp04Same('3.1.0',$contract['openapi']??null);
    foreach(['400','401','403','404','405','409','415','422','429','500'] as $status)wp04Assert(isset($contract['x-freeitsm-status-semantics'][$status]),'Missing '.$status);
    wp04Assert(isset($contract['paths']['/session']));
    wp04Assert(isset($contract['paths']['/session/tenant']));
});
