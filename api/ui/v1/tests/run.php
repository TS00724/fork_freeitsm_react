...
$testRoutes = array_merge($foundationRoutes, [
    new UiApiRoute('POST', '#^/echo$#', 'test.echo', static function (UiApiRequest $request): array {
...
wp04Test('HEAD uses GET routing and emits no body', static function () use ($kernel): void {
    ...
    wp04Same(200, $response->status()); wp04Same('', $response->body());
});
wp04Test('OPTIONS returns a route-specific Allow header', static function () use ($kernel): void {
    ...
    wp04Same(204, $response->status()); wp04Same('GET, HEAD, OPTIONS', $response->header('Allow')); wp04Same('', $response->body());
});
wp04Test('an unknown path returns 404 with the common envelope', static function () use ($kernel): void {
    ...
    wp04Same(404, $response->status()); $json = wp04Json($response);
    wp04Same('not_found', $json['error']['code']); wp04Assert(isset($json['meta']['requestId'], $json['meta']['correlationId']));
});
wp04Test('a known route with POST returns 405 and Allow', static function () use ($kernel): void {
    ...
    wp04Same(405, $response->status()); wp04Same('GET, HEAD, OPTIONS', $response->header('Allow'));
...
wp04Test('TRACE is not silently accepted', static function () use ($kernel): void {
    wp04Same(405, $kernel->handleServer(wp04Server('TRACE', '/'))->status());
});
wp04Test('malformed JSON returns 400', static function () use ($kernel): void {
    ...
    wp04Same(400, $response->status()); wp04Same('invalid_json', wp04Json($response)['error']['code']);
});
wp04Test('a top-level JSON array is rejected', static function () use ($kernel): void {
    wp04Same(400, $kernel->handleServer(wp04Server('POST', '/echo', ['Content-Type' => 'application/json']), [], '[]')->status());
});
wp04Test('a top-level JSON scalar is rejected', static function () use ($kernel): void {
    wp04Same(400, $kernel->handleServer(wp04Server('POST', '/echo', ['Content-Type' => 'application/json']), [], 'true')->status());
});
wp04Test('unsupported Content-Type returns 415', static function () use ($kernel): void {
    ...
    wp04Same(415, $response->status()); wp04Same('unsupported_media_type', wp04Json($response)['error']['code']);
});
wp04Test('application/json with parameters is accepted', static function () use ($kernel): void {
    $response = $kernel->handleServer(wp04Server('POST', '/echo', ['Content-Type' => 'application/json; charset=utf-8']), [], '{"name":"test"}');
    wp04Same(200, $response->status()); wp04Same('test', wp04Json($response)['data']['body']['name']);
});
wp04Test('application vendor +json is accepted', static function () use ($kernel): void {
    $response = $kernel->handleServer(wp04Server('POST', '/echo', ['Content-Type' => 'application/vnd.freeitsm+json']), [], '{"accepted":true}');
    wp04Same(200, $response->status()); wp04Same(true, wp04Json($response)['data']['body']['accepted']);
});
wp04Test('an empty optional command body becomes an empty object', static function () use ($kernel): void {
    wp04Same([], wp04Json($kernel->handleServer(wp04Server('POST', '/echo'), [], ''))['data']['body']);
});
wp04Test('a valid request ID is propagated into header and body', static function () use ($kernel): void {
    $response = $kernel->handleServer(wp04Server('GET', '/', ['X-Request-ID' => 'request-123'])); $json = wp04Json($response);
    wp04Same('request-123', $response->header('X-Request-ID')); wp04Same('request-123', $json['meta']['requestId']);
});
...
wp04Test('a valid correlation ID is propagated', static function () use ($kernel): void {
    $response = $kernel->handleServer(wp04Server('GET', '/', ['X-Request-ID' => 'request-1', 'X-Correlation-ID' => 'correlation-1']));
    wp04Same('correlation-1', $response->header('X-Correlation-ID')); wp04Same('correlation-1', wp04Json($response)['meta']['correlationId']);
});
wp04Test('correlation ID defaults to request ID', static function () use ($kernel): void {
    wp04Same('request-2', $kernel->handleServer(wp04Server('GET', '/', ['X-Request-ID' => 'request-2']))->header('X-Correlation-ID'));
});
wp04Test('envelope IDs exactly match response headers', static function () use ($kernel): void {
    $response = $kernel->handleServer(wp04Server('GET', '/missing')); $json = wp04Json($response);
...
wp04Test('a raw backslash in the path is rejected', static function () use ($kernel): void {
    ...
    wp04Same(400, $response->status()); wp04Same('invalid_path', wp04Json($response)['error']['code']);
});
...
wp04Test('an invalid typed route parameter returns 400', static function () use ($kernel): void {
    ...
    wp04Same(400, $response->status()); wp04Same('invalid_route_parameter', wp04Json($response)['error']['code']);
});
wp04Test('a valid typed route parameter is coerced', static function () use ($kernel): void {
    wp04Same(42, wp04Json($kernel->handleServer(wp04Server('GET', '/items/42')))['data']['id']);
});
wp04Test('an internal exception does not leak message, path or stack', static function () use ($kernel): void {
    $response = $kernel->handleServer(wp04Server('GET', '/boom')); wp04Same(500, $response->status()); $body = $response->body();
    wp04Assert(strpos($body, 'SECRET_TOKEN') === false); wp04Assert(strpos($body, '/private/server/path.php') === false);
    wp04Assert(strpos(strtolower($body), 'trace') === false); wp04Same('server_error', wp04Json($response)['error']['code']);
});
wp04Test('401 contract semantics use unauthenticated', static function () use ($kernel): void {
    ...
    wp04Same(401, $response->status()); wp04Same('unauthenticated', wp04Json($response)['error']['code']);
});
wp04Test('403 contract semantics use forbidden', static function () use ($kernel): void {
    ...
    wp04Same(403, $response->status()); wp04Same('forbidden', wp04Json($response)['error']['code']);
});
wp04Test('409 contract semantics preserve safe conflict detail', static function () use ($kernel): void {
    ...
    wp04Same(409, $response->status()); wp04Same('version', wp04Json($response)['error']['details']['field']);
});
wp04Test('422 contract semantics use validation_failed', static function () use ($kernel): void {
    ...
    wp04Same(422, $response->status()); wp04Same('validation_failed', wp04Json($response)['error']['code']);
});
wp04Test('429 includes Retry-After', static function () use ($kernel): void {
    $response = $kernel->handleServer(wp04Server('GET', '/status/429'));
    wp04Same(429, $response->status()); wp04Same('15', $response->header('Retry-After'));
});
wp04Test('responses never grant wildcard CORS', static function () use ($kernel): void {
    $response = $kernel->handleServer(wp04Server('GET', '/'));
    wp04Same(null, $response->header('Access-Control-Allow-Origin')); wp04Same(null, $response->header('Access-Control-Allow-Credentials'));
});
wp04Test('production PHP does not start Session, DB or machine API auth', static function (): void {
    $root = dirname(__DIR__); $phpFiles = array_merge(glob($root . '/*.php') ?: [], glob($root . '/lib/*.php') ?: []); $source = '';
    ...
    wp04Assert(stripos($source, 'config.php') === false, 'WP-04 production PHP must not load DB config.');
});
wp04Test('OpenAPI source declares every required status semantic', static function (): void {
    $contract = json_decode((string)file_get_contents(dirname(__DIR__) . '/openapi.json'), true);
    wp04Assert(is_array($contract)); wp04Same('3.1.0', $contract['openapi']);
