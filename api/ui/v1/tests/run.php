...
testCase('root success envelope and metadata', function () use ($kernel): void {
    $response = $kernel->handle(new UiRequest(
        'GET',
        '/',
...
$jsonRoute = new UiRoute(
    'POST',
    '/parse',
    'testParse',
    function (UiRequest $request, UiRequestContext $context, array $params): UiRouteResult {
...
testCase('malformed JSON is 400', function () use ($jsonKernel): void {
    $response = $jsonKernel->handle(new UiRequest(
        'POST',
        '/parse',
...
foreach ([
    ...
    testCase("HTTP {$status} semantic envelope", function () use ($status, $code): void {
        $route = new UiRoute(
            ...
            '/status',
            'testStatus',
            function (UiRequest $request, UiRequestContext $context, array $params) use ($status, $code): UiRouteResult {
