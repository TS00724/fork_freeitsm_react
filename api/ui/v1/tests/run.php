...
function decodeResponse($response): array
{
    ...
    if (!is_array($decoded)) throw new RuntimeException('Response is not JSON object');
    return $decoded;
}
...
$jsonRoute = new UiRoute('POST', '/parse', 'testParse', function (UiRequest $request): UiRouteResult {
...
testCase('valid JSON object is parsed', function () use ($jsonKernel): void {
...
$explodeRoute = new UiRoute('GET', '/explode', 'testExplode', function (): UiRouteResult {
...
testCase('internal exception does not leak stack, path or secret', function () use ($explodeKernel): void {
    ...
    assertTrue(strpos($bodyText, '/srv/private') === false);
    assertTrue(strpos($bodyText, 'secret-token-123') === false);
...
foreach ([401 => 'unauthenticated', 403 => 'forbidden', 409 => 'conflict', 422 => 'validation_failed', 429 => 'rate_limited'] as $status => $code) {
    testCase("HTTP {$status} semantic envelope", function () use ($status, $code): void {
        $route = new UiRoute('GET', '/status', 'testStatus', function () use ($status, $code): UiRouteResult {
...
testCase('path traversal and encoded separators are rejected', function (): void {
    foreach (['/../health', '/%252e%252e/health', '/a%2Fb'] as $path) {
        try {
...
testCase('UI API source has no wildcard CORS or machine API authentication', function (): void {
