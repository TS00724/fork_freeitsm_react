...
function wp04Server(string $method, string $path, array $headers = []): array {
    $server = ['REQUEST_METHOD'=>$method,'PATH_INFO'=>$path,'REQUEST_URI'=>'/api/ui/v1/index.php'.$path,'SCRIPT_NAME'=>'/api/ui/v1/index.php','HTTP_HOST'=>'desk.example.test','HTTPS'=>'on','SERVER_PORT'=>443];
...
wp04Test('production UI API stays separate from machine and business APIs',static function():void{$root=dirname(__DIR__);$source='';foreach(array_merge(glob($root.'/*.php')?:[],glob($root.'/lib/*.php')?:[])as$file)$source.=file_get_contents($file)?:'';$code='';foreach(token_get_all($source)as$token){if(is_array($token)&&($token[0]===T_COMMENT||$token[0]===T_DOC_COMMENT))continue;$code.=is_array($token)?$token[1]:$token;}wp04Assert(stripos($code,'HTTP_AUTHORIZATION')===false,'Machine Authorization header detected.');wp04Assert(stripos($code,'Bearer ')===false,'Bearer API key detected.');foreach(['calendar','watchtower','ticket','asset','knowledge','cmdb']as$feature)wp04Assert(stripos($code,"#^/$feature")===false,'Business route detected: '.$feature);});
wp04Test('OpenAPI declares required semantics',static function():void{$c=json_decode((string)file_get_contents(dirname(__DIR__).'/openapi-v1.json'),true);wp04Same('3.1.0',$c['openapi']??null);foreach(['400','401','403','404','405','409','415','422','429','500']as$s)wp04Assert(isset($c['x-freeitsm-status-semantics'][$s]),'Missing '.$s);wp04Assert(isset($c['paths']['/session']));wp04Assert(isset($c['paths']['/session/tenant']));});
