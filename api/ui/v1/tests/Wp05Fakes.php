...
final class Wp05FakeSecurityProvider implements UiApiSecurityProvider
{
    ...
    public function links(): array
    {
        return [
            'login' => '/login',
            'logout' => '/logout',
            'passwordChange' => '/auth/force_password_change.php',
        ];
    }
...
function wp05Runtime(
    Wp05ArraySessionStore $session,
    ?Wp05FakeSecurityProvider $provider = null
): array {
    $provider = $provider ?? new Wp05FakeSecurityProvider([
        7 => Wp05FakeSecurityProvider::state(),
    ]);
