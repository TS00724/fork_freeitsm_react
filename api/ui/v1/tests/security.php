...
function wp05Kernel(Wp05ArraySessionStore $session, ?Wp05FakeSecurityProvider $provider = null): array
{
    [$runtime, $provider] = wp05Runtime($session, $provider);
    $routes = array_merge(uiApiRoutes($runtime), [
        new UiApiRoute('GET', '#^/protected$#', 'test.protected', static fn(): array => ['ok' => true], [], UiApiRouteSecurity::authenticated()),
        new UiApiRoute('POST', '#^/write$#', 'test.write', static fn(): array => ['ok' => true], [], UiApiRouteSecurity::authenticatedWrite()),
        new UiApiRoute('GET', '#^/capability$#', 'test.capability', static fn(): array => ['ok' => true], [], UiApiRouteSecurity::authenticated()->requiringCapability('tickets.manage')),
        new UiApiRoute('GET', '#^/module$#', 'test.module', static fn(): array => ['ok' => true], [], UiApiRouteSecurity::authenticated()->requiringModule('tickets')),
        new UiApiRoute('GET', '#^/tenants/(?P<tenantId>[^/]+)$#', 'test.tenant', static fn(): array => ['ok' => true], ['tenantId' => 'positive_int'], UiApiRouteSecurity::authenticated()->requiringTenantParameter('tenantId')),
        new UiApiRoute('GET', '#^/objects/(?P<id>[^/]+)$#', 'test.object', static fn(): array => ['ok' => true], ['id' => 'positive_int'], UiApiRouteSecurity::authenticated()->requiringObjectScope(static fn(array $state, array $parameters): bool => (int) $parameters['id'] === 1)),
    ]);
...
{
    $response = $kernel->handleServer(wp04Server('GET', '/session'));
    return (string) (wp04Json($response)['data']['csrf']['token'] ?? '');
}

wp04Test('WP-05 anonymous session probe', static function (): void {
    [$kernel] = wp05Kernel(new Wp05ArraySessionStore());
    $json = wp04Json($kernel->handleServer(wp04Server('GET', '/session')));
    wp04Same(false, $json['data']['authenticated']);
    wp04Assert(!isset($json['data']['csrf']));
});
...
wp04Test('WP-05 authenticated bootstrap is bounded', static function (): void {
    ...
    wp04Same(true, $json['data']['authenticated']);
    wp04Same('oidc', $json['data']['actor']['authSource']);
    wp04Assert(strpos($body, 'private-session-id') === false);
    wp04Assert(preg_match('/^[a-f0-9]{64}$/', $json['data']['csrf']['token']) === 1);
});
...
wp04Test('WP-05 password-change Session is blocked', static function (): void {
    [$kernel] = wp05Kernel(new Wp05ArraySessionStore(['analyst_id' => 7, 'password_expired' => true]));
    $response = $kernel->handleServer(wp04Server('GET', '/protected'));
    wp04Same(403, $response->status());
    wp04Same('password_change_required', wp04Json($response)['error']['code']);
});

wp04Test('WP-05 missing Origin fails before DB resolution', static function (): void {
    ...
    $response = $kernel->handleServer(wp04Server('POST', '/write', ['X-CSRF-Token' => $token]), [], '{}');
    wp04Same(403, $response->status());
...
wp04Test('WP-05 foreign Origin is rejected', static function (): void {
    ...
    [$kernel] = wp05Kernel($session);
    $token = wp05Csrf($kernel);
    $response = $kernel->handleServer(wp04Server('POST', '/write', ['Origin' => 'https://evil.example', 'X-CSRF-Token' => $token]), [], '{}');
...
wp04Test('WP-05 missing CSRF token is rejected', static function (): void {
    ...
    wp05Csrf($kernel);
    $response = $kernel->handleServer(wp04Server('POST', '/write', ['Origin' => 'https://desk.example.test']), [], '{}');
...
wp04Test('WP-05 invalid CSRF token is rejected', static function (): void {
    [$kernel] = wp05Kernel(new Wp05ArraySessionStore(['analyst_id' => 7]));
    wp05Csrf($kernel);
    $response = $kernel->handleServer(wp04Server('POST', '/write', ['Origin' => 'https://desk.example.test', 'X-CSRF-Token' => str_repeat('a', 64)]), [], '{}');
    wp04Same(403, $response->status());
});
...
wp04Test('WP-05 Referer fallback accepts same origin', static function (): void {
    [$kernel] = wp05Kernel(new Wp05ArraySessionStore(['analyst_id' => 7]));
    $token = wp05Csrf($kernel);
    $response = $kernel->handleServer(wp04Server('POST', '/write', ['Referer' => 'https://desk.example.test/ui/', 'X-CSRF-Token' => $token]), [], '{}');
...
wp04Test('WP-05 exact Origin and token allow write', static function (): void {
    [$kernel] = wp05Kernel(new Wp05ArraySessionStore(['analyst_id' => 7]));
    $token = wp05Csrf($kernel);
    $response = $kernel->handleServer(wp04Server('POST', '/write', ['Origin' => 'https://desk.example.test', 'X-CSRF-Token' => $token]), [], '{}');
    wp04Same(200, $response->status());
});

wp04Test('WP-05 capability policy is authoritative', static function (): void {
    $state = Wp05FakeSecurityProvider::state(['capabilities' => []]);
    [$denied] = wp05Kernel(new Wp05ArraySessionStore(['analyst_id' => 7]), new Wp05FakeSecurityProvider([7 => $state]));
...
wp04Test('WP-05 module policy denies missing module', static function (): void {
    ...
    [$kernel] = wp05Kernel(new Wp05ArraySessionStore(['analyst_id' => 7]), new Wp05FakeSecurityProvider([7 => $state]));
...
wp04Test('WP-05 tenant switch validates input', static function (): void {
    [$kernel] = wp05Kernel(new Wp05ArraySessionStore(['analyst_id' => 7]));
    $token = wp05Csrf($kernel);
    $response = $kernel->handleServer(wp04Server('POST', '/session/tenant', ['Origin' => 'https://desk.example.test', 'X-CSRF-Token' => $token, 'Content-Type' => 'application/json']), [], '{"tenantId":0}');
...
wp04Test('WP-05 tenant switch denies inaccessible company', static function (): void {
    [$kernel] = wp05Kernel(new Wp05ArraySessionStore(['analyst_id' => 7]));
    $token = wp05Csrf($kernel);
    $response = $kernel->handleServer(wp04Server('POST', '/session/tenant', ['Origin' => 'https://desk.example.test', 'X-CSRF-Token' => $token, 'Content-Type' => 'application/json']), [], '{"tenantId":99}');
    wp04Same(403, $response->status());
});
...
wp04Test('WP-05 tenant switch updates context and rotates CSRF', static function (): void {
    [$kernel] = wp05Kernel(new Wp05ArraySessionStore(['analyst_id' => 7]));
    $token = wp05Csrf($kernel);
    $response = $kernel->handleServer(wp04Server('POST', '/session/tenant', ['Origin' => 'https://desk.example.test', 'X-CSRF-Token' => $token, 'Content-Type' => 'application/json']), [], '{"tenantId":2}');
    $json = wp04Json($response);
    wp04Same(2, $json['data']['activeTenant']['id']);
    wp04Assert($token !== $json['data']['csrf']['token']);
});
